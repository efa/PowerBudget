#!/bin/bash
# PowerBudget v0.00.01a 2025/08/18 calculate power dissipation and budget
...
BINRAD="powerBudget" # binary radix
BINPATH=`pwd`/.. # where find the just build binary
SRCPATH="." # source package path
RESPATH=".." # other resources (readme, icons, desktop files) to copy in packages
TMPPATH="../pkg" # temp path where create uncompressed dir,AppDir,DiskImage
DSTPATH="../.." # path where write the final Linux|Mingw/MXE|OSX package
...
echo "makePkg.sh: create a Linux|WinMxe|WinMgw|Osx package for $APP ..."
...
BINCLI="${BINRAD}$PKG$BIT"
BINGUI="${BINRAD}Gui$PKG$BIT"
...
if [[ "$OS" != "Msys" && "$OS" != "GNU/Linux" ]]; then
   echo "ERROR makePkg.sh: work in Linux|MXE(Linux)|WinMgw only"
   exit
fi
...
DATE=`date -I`
VER=`grep SourceVersion powerbLib.h | cut -d' ' -f3 | tr -d '."'` # M.mm.bb ==> Mmmbb
TGT=$PKG
if [[ "$OS" = "Msys" ]]; then
   if [[ "$BIT" = "64" ]]; then
...
echo "SRCPATH: $SRCPATH" # sources path
...
if (test "$TGT" = "Osx") then
   if (test "$BIT" = "32") then
      echo "Unsupported 32 bit on Osx"
      exit
   fi
   if (test "$BINCLI" != "") then
...
cp -a $SRCPATH/makePkg.sh $TMPPATH/$APP/src/
cp -a $SRCPATH/*.ini      $TMPPATH/$APP/src/   # if needed
#cp -aL $SRCPATH/$APP.png   $TMPPATH/$APP/src/   # if needed

echo "Copying assets ..."
cp -a $RESPATH/powerb.ini   $TMPPATH/$APP/   # if needed
if (test "$TGT" = "Linux") then
cp -a $RESPATH/powerb.ini   $TMPPATH/AppDir/$TMPBINP   # if needed
...
echo "Copying docs ..."
cp -a $RESPATH/COPYING       $TMPPATH/$APP/
...
if (test "$TGT" = "Linux") then
cp -a $RESPATH/COPYING       $TMPPATH/AppDir/
...
if [[ ("$TGT" = "Linux") && ("$CPU" = "x86_64" || "$CPU" = "i686")]]; then # skip on ARM & RISC-V
   ...
   if (test "$BIT" = "64") then
      ...
      #fi
      #pwd
      ./linuxdeploy-x86_64.AppImage -e $TMPPATH/AppDir/$TMPBINP/$BINGUI --appdir AppDir -i $TMPPATH/AppDir/$APP.png -d $TMPPATH/AppDir/$APP.desktop --output appimage > logLinuxdeploy$DATE.txt
      ret=$?
      #file=${APP}_${VER}_${DATE}_${TGT}_${CPU}_${BIT}bit.AppImage
      file="$PKGNAME.AppImage"
      if (test "$ret" = "0") then
         echo "AppImage created: $DSTPATH/$file"
         mv ${APP}Gui-x86_64.AppImage $DSTPATH/$file
      else
         echo "AppImage failed: $DSTPATH/$file"
      fi
   fi
   if (test "$BIT" = "32") then
